...
app.post("/sign-in", async (req, res) => {
  ...
  try {
    const [rows] = await sqlPool.query(sql, [email, psw]);
    if (rows.length > 0) {
      const user = {
...
app.post("/sign-up", async (req, res) => {
  ...
  try {
    const [result] = await sqlPool.query(sql, [fullname, email, psw]);
    console.log(result);
...
app.get("/get/email/:option/:id", async (req, res) => {
  ...
  try {
    ...
    const [countResult] = await sqlPool.query(countSql, [id]);
    const totalEmails = countResult[0].total;
    const totalPages = Math.ceil(totalEmails / limit);

    const [rows] = await sqlPool.query(sql, [id, limit, offset]);
...
app.get("/get/email-detail/:id", async (req, res) => {
  ...
  try {
    const [rows] = await sqlPool.query(sql, [emailId]);
    if (rows.length > 0) {
      const email = rows[0];
      // Handle null subject
...
app.get("/get/all-available-emails", async (req, res) => {
  ...
  try {
    const [rows] = await sqlPool.query(sql, [user.userId]);
    if (rows.length > 0) {
...
app.get("/get/:email", async (req, res) => {
  ...
  try {
    const [rows] = await sqlPool.query(sql, [req.params.email]);
    return res.json(rows);
  } catch (err) {
    res.status(500).json({ error: "Database error" });
  }
